...
where `vm` is the package `"github.com/lumoschain/lumos/v20/x/evm/core/vm"`.
...
[`x/evm/`](https://github.com/lumoschain/lumos/tree/main/x/evm) folder
of **evmOS**.
...
lumosActivators = map[int]func(*vm.JumpTable){
...
```go
lumosEnabledEIPs = []int64{
    "lumos_0",
}
```

In this way, even though the custom activators defined $3$ new EIPs, we are going to activate only the number `lumos_0`
