...
import (
	...
	evm "github.com/lumoschain/lumos/v20/x/evm/types"
)
...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the alumos amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	chainID := lumosApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := lumosApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: lumosApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(lumosApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, lumosApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := lumosApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := lumosApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := lumosApp.EvmKeeper.GetNonce(ctx, from)
