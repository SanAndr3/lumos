...
def test_gas_eth_tx(geth, lumos_cluster):
    tx_value = 10
    ...
    # send an equivalent transaction with lumos
    lumos_gas_price = lumos_cluster.w3.eth.gas_price
    tx = {"to": ADDRS["community"], "value": tx_value, "gasPrice": lumos_gas_price}
    lumos_receipt = send_transaction(lumos_cluster.w3, tx, KEYS["validator"])

    assert geth_receipt.gasUsed == lumos_receipt.gasUsed


def test_gas_deployment(geth, lumos_cluster):
    # deploy an identical contract on geth and lumos
    ...
    lumos_tx = build_deploy_contract_tx(lumos_cluster.w3, info)

    # estimate tx gas
    geth_gas_estimation = geth.w3.eth.estimate_gas(geth_tx)
    lumos_gas_estimation = lumos_cluster.w3.eth.estimate_gas(lumos_tx)

    assert geth_gas_estimation == lumos_gas_estimation

    # sign and send tx
    geth_contract_receipt = send_transaction(geth.w3, geth_tx)
    lumos_contract_receipt = send_transaction(lumos_cluster.w3, lumos_tx)
    assert geth_contract_receipt.status == 1
    assert lumos_contract_receipt.status == 1

    assert geth_contract_receipt.gasUsed == lumos_contract_receipt.gasUsed

    # gasUsed should be same as estimation
    assert geth_contract_receipt.gasUsed == geth_gas_estimation
    assert lumos_contract_receipt.gasUsed == lumos_gas_estimation


def test_gas_call(geth, lumos_cluster):
    function_input = 10
    ...
    lumos_contract, _ = deploy_contract(lumos_cluster.w3, CONTRACTS["BurnGas"])
    ...
    # repeat the above for lumos
    lumos_gas_price = lumos_cluster.w3.eth.gas_price
    lumos_tx = lumos_contract.functions.burnGas(function_input).build_transaction(
        {"from": ADDRS["validator"], "gasPrice": lumos_gas_price}
    )
    lumos_gas_estimation = lumos_cluster.w3.eth.estimate_gas(lumos_tx)
    lumos_call_receipt = send_transaction(lumos_cluster.w3, lumos_tx)

    # ensure gas estimation is the same
    assert geth_gas_estimation == lumos_gas_estimation

    # ensure that the gasUsed is equivalent
    assert geth_call_receipt.gasUsed == lumos_call_receipt.gasUsed

    # ensure gasUsed == gas estimation
    assert geth_call_receipt.gasUsed == geth_gas_estimation
    assert lumos_call_receipt.gasUsed == lumos_gas_estimation


def test_block_gas_limit(lumos_cluster):
    tx_value = 10

    # get the block gas limit from the latest block
    w3_wait_for_new_blocks(lumos_cluster.w3, 5)
    block = lumos_cluster.w3.eth.get_block("latest")
    ...
    try:
        send_transaction(lumos_cluster.w3, tx, KEYS["validator"])
    except Exception as error:
        assert "exceeds block gas limit" in error.args[0]["message"]

    # deploy a contract on lumos
    lumos_contract, _ = deploy_contract(lumos_cluster.w3, CONTRACTS["BurnGas"])

    # expect an error on contract call due to block gas limit
    try:
        burn_gas_tx = lumos_contract.functions.burnGas(
            exceeded_gas_limit
        ).build_transaction(
            {
        ...
        send_transaction(lumos_cluster.w3, burn_gas_tx, KEYS["validator"])
    except Exception as error:
        assert "exceeds block gas limit" in error.args[0]["message"]
