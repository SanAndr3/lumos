...
LUMOS_ADDRESS_PREFIX = "lumos"
DEFAULT_DENOM = "alumos"
WLUMOS_ADDRESS = Web3.toChecksumAddress("0xD4949664cD82660AaE99bEdc034a0deA8A0bd517")
TEST_CONTRACTS = {
    ...
    "InterchainSender": "lumos/testutil/contracts/InterchainSender.sol",
    "InterchainSenderCaller": "lumos/testutil/contracts/InterchainSenderCaller.sol",
    "ICS20I": "lumos/ics20/ICS20I.sol",
    "DistributionI": "lumos/distribution/DistributionI.sol",
    "DistributionCaller": "lumos/testutil/contracts/DistributionCaller.sol",
    "StakingI": "lumos/staking/StakingI.sol",
    "StakingCaller": "lumos/staking/testdata/StakingCaller.sol",
    "IStrideOutpost": "lumos/outposts/stride/IStrideOutpost.sol",
    "IOsmosisOutpost": "lumos/outposts/osmosis/IOsmosisOutpost.sol",
    "IERC20": "lumos/erc20/IERC20.sol",
}

OSMOSIS_POOLS = {
    "Lumos_Osmo": Path(__file__).parent / "osmosis/lumosOsmosisPool.json",
}
...
REGISTER_ERC20_PROP = {
    "messages": [
        {
            "@type": "/lumos.erc20.v1.MsgRegisterERC20",
...
def eth_to_bech32(addr, prefix=LUMOS_ADDRESS_PREFIX):
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:lumos_7798-1-node{i}":
            ini[section].update(
                {
...
def update_lumosd_and_setup_stride(modified_bin):
    def inner(path, base_port, config):  # pylint: disable=unused-argument
        update_lumos_bin(modified_bin)(path, base_port, config)
        setup_stride()(path, base_port, config)

    return inner


def update_lumos_bin(
    modified_bin, nodes=[0, 1]
):  # pylint: disable=dangerous-default-value
    """
    updates the lumos binary with a patched binary.
...
def debug_trace_tx(lumos, tx_hash: str):
    url = f"http://127.0.0.1:{ports.evmrpc_port(lumos.base_port(0))}"
    params = {
