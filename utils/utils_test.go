...
func TestGetLumosAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"lumos address",
			"lumos1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"lumos1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetLumosAddressFromBech32(tc.address)
		if tc.expError {
...
func TestLumosCoinDenom(t *testing.T) {
	testCases := []struct {
		name     string
		denom    string
		expError bool
