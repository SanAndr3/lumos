...
func CalculateEpochMintProvision(
	...
	// Multiply epochMintProvision with power reduction (10^18 for lumos) as the
	// calculation is based on `lumos` and the issued tokens need to be given in
	// `alumos`
	epochProvision = epochProvision.Mul(math.LegacyNewDecFromInt(lumostypes.PowerReduction))
