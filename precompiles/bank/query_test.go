...
import (
	...
	lumosutiltx "github.com/lumoschain/lumos/v20/testutil/tx"
)

func (s *PrecompileTestSuite) TestBalances() {
	var ctx sdk.Context
	// setup test in order to have s.precompile, s.lumosAddr and s.xmplAddr defined
	s.SetupTest()
	...
	testcases := []struct {
		...
		expPass     bool
		errContains string
		expBalances func(lumosAddr, xmplAddr common.Address) []bank.Balance
	}{
		{
		...
		},
		{
			...
			true,
			"",
			func(lumosAddr, xmplAddr common.Address) []bank.Balance {
				return []bank.Balance{
					{
						ContractAddress: lumosAddr,
		...
		{
			"pass - LUMOS and XMPL balances present - mint extra XMPL",
			func() []interface{} {
	...
	for _, tc := range testcases {
		...
		s.Run(tc.name, func() {
			...
			if tc.expPass {
				...
				s.Require().NoError(err)
				s.Require().Equal(tc.expBalances(s.lumosAddr, s.xmplAddr), balances)
			} else {
...
func (s *PrecompileTestSuite) TestTotalSupply() {
	...
	s.Require().NoError(err)
	lumosTotalSupply := totSupplRes.Supply.AmountOf(s.bondDenom)
	xmplTotalSupply := totSupplRes.Supply.AmountOf(s.tokenDenom)

	testcases := []struct {
		name      string
		malleate  func()
		expSupply func(lumosAddr, xmplAddr common.Address) []bank.Balance
	}{
		{
			"pass - LUMOS and XMPL total supply",
			func() {
				ctx = s.mintAndSendXMPLCoin(ctx, s.keyring.GetAccAddr(0), math.NewInt(1e18))
			},
			func(lumosAddr, xmplAddr common.Address) []bank.Balance {
				return []bank.Balance{{
					ContractAddress: lumosAddr,
					Amount:          lumosTotalSupply.BigInt(),
				}, {
					ContractAddress: xmplAddr,
	...
	for _, tc := range testcases {
		...
		s.Run(tc.name, func() {
			...
			err = s.precompile.UnpackIntoInterface(&balances, method.Name, bz)
			s.Require().NoError(err)
			s.Require().Equal(tc.expSupply(s.lumosAddr, s.xmplAddr), balances)
		})
	}
}
