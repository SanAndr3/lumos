...
// RegisterLumosERC20Coins uses the UnitNetwork to register the lumos token as an
...
func RegisterLumosERC20Coins(
	...
	lumosMetadata, found := network.App.BankKeeper.GetDenomMetaData(network.GetContext(), utils.BaseDenom)
	if !found {
		return erc20types.TokenPair{}, fmt.Errorf("expected lumos denom metadata")
	}

	_, err = network.App.Erc20Keeper.RegisterERC20Extension(network.GetContext(), lumosMetadata.Base)
	if err != nil {
		return erc20types.TokenPair{}, err
	}

	lumosDenomID := network.App.Erc20Keeper.GetDenomMap(network.GetContext(), bondDenom)
	tokenPair, ok := network.App.Erc20Keeper.GetTokenPair(network.GetContext(), lumosDenomID)
	if !ok {
		return erc20types.TokenPair{}, fmt.Errorf("expected lumos erc20 token pair")
	}
