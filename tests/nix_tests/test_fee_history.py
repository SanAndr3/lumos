...
def custom_lumos(tmp_path_factory):
    ...
    yield from setup_lumos(path, 26500)


@pytest.fixture(scope="module")
def custom_lumos_rocksdb(tmp_path_factory):
    path = tmp_path_factory.mktemp("fee-history-rocksdb")
    yield from setup_lumos_rocksdb(path, 26510)


@pytest.fixture(scope="module", params=["lumos", "lumos-rocksdb", "geth"])
def cluster(request, custom_lumos, custom_lumos_rocksdb, geth):
    """
    run on lumos, lumos built with rocksdb and geth
    """
