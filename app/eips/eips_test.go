...
import (
	...
	"github.com/lumoschain/lumos/v20/app/eips/testdata"
	...
	integrationutils "github.com/lumoschain/lumos/v20/testutil/integration/lumos/utils"
...
func TestIPs(t *testing.T) {
	...
	RunSpecs(t, "LumosIPs Suite")
}

var _ = Describe("Improvement proposal lumos_0 - ", Ordered, func() {
	var (
		in network.Network
	...
	// Multiplier used to modify the opcodes associated with lumos_0 IP.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have IP lumos_0 in evm params")
	})

	It("should change CREATE opcode constant gas after enabling lumos_0 IP", func() {
...
var _ = Describe("Improvement proposal lumos_1 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Multiplier used to modify the opcodes associated with lumos_1.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip lumos_1 in evm params")
	})
...
var _ = Describe("Improvement proposal lumos_2 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Constant gas used to modify the opcodes associated with lumos_2.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip lumos_2 in evm params")
	})
