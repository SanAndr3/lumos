...
def custom_lumos(tmp_path_factory):
    ...
    yield from setup_lumos(path, 26800, long_timeout_commit=True)
...
def custom_lumos_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_lumos_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(lumos_cluster):
    """
...
def test_native_tx_priority(lumos_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000alumos",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}alumos",
        ...
        },
        {
            ...
            "amount": "1000alumos",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}alumos",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("alumos"))
