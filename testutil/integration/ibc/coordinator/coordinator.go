...
import (
	...
	lumosibc "github.com/lumoschain/lumos/v20/ibc/testing"
	"github.com/lumoschain/lumos/v20/testutil/integration/common/network"
	ibcchain "github.com/lumoschain/lumos/v20/testutil/integration/ibc/chain"
)
...
type Coordinator interface {
	...
	GetPath(a, b string) *lumosibc.Path
...
// GetPath returns the transfer path for the chain ids 'a' and 'b'
func (c *IntegrationCoordinator) GetPath(a, b string) *lumosibc.Path {
	chainA := c.coord.GetChain(a)
	chainB := c.coord.GetChain(b)

	return lumosibc.NewTransferPath(chainA, chainB)
}
...
func (c *IntegrationCoordinator) Setup(a, b string) IBCConnection {
	path := c.GetPath(a, b)
	lumosibc.SetupPath(c.coord, path)
