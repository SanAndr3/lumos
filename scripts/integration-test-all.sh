...
## default port prefixes for lumosd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/lumosd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/lumosd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/lumosd add-genesis-account \
    "$("$PWD"/build/lumosd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000alumos,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/lumosd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/lumosd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/lumosd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/lumosd start --pruning=nothing --rpc.unsafe \
    ...
    LUMOS_PID=$!
    echo "started lumos node, pid=$LUMOS_PID"
    # add PID to array
    arr+=("$LUMOS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test lumos node $HOST_RPC ..."
...
stop_func() {
    LUMOS_PID=$i
    echo "shutting down node, pid=$LUMOS_PID ..."
