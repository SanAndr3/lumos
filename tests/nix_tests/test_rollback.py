...
from .network import build_patched_lumosd, setup_custom_lumos
from .utils import (
    supervisorctl,
    update_lumos_bin,
    update_node_cmd,
...
def custom_lumos(tmp_path_factory):
    ...
    broken_binary = build_patched_lumosd("broken-lumosd")
    print(broken_binary)
    ...
    yield from setup_custom_lumos(
        ...
        post_init=update_lumos_bin(broken_binary, [1]),
        wait_port=False,
    )


def test_rollback(custom_lumos):
    """
    ...
    print("stop node1")
    supervisorctl(custom_lumos.base_dir / "../tasks.ini", "stop", "lumos_7798-1-node1")

    print("do rollback on node1")
    cli1.rollback()

    print("switch to normal binary")
    update_node_cmd(custom_lumos.base_dir, "lumosd", 1)
    supervisorctl(custom_lumos.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
