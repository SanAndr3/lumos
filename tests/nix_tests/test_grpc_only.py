...
from .network import Lumos, create_snapshots_dir, setup_custom_lumos
from .utils import (
    CONTRACTS,
    decode_bech32,
    deploy_contract,
...
def lumos_cluster(request, custom_lumos, custom_lumos_rocksdb):
    """
    run on lumos and
...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/lumos/evm/v1/eth_call", params).json()


def test_grpc_mode(lumos_cluster: Lumos):
    """
    ...
    api_port = ports.api_port(lumos_cluster.base_port(1))
    ...
    for i in range(2):
        wait_for_block(lumos_cluster.cosmos_cli(i), 1)
    supervisorctl(lumos_cluster.base_dir / "../tasks.ini", "stop", "lumos_7798-1-node1")

    # run grpc-only mode directly with existing chain state
    with open(lumos_cluster.base_dir / "node1.log", "a", encoding="utf-8") as logfile:
        proc = subprocess.Popen(  # pylint: disable=consider-using-with
            [
                lumos_cluster.chain_binary,
                "start",
                "--grpc-only",
                "--home",
                lumos_cluster.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            grpc_port = ports.grpc_port(lumos_cluster.base_port(1))
            ...
            addr = lumos_cluster.cosmos_cli(0).consensus_address()
