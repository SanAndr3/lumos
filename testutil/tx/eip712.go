...
func PrepareEIP712CosmosTx(
	...
	accNumber := appLumos.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appLumos.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		lumosCodec := codec.NewProtoCodec(registry)
