...
def test_ibc_transfer_with_hermes(ibc):
    ...
    old_dst_balance = get_balance(ibc.chains["lumos"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["lumos"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("alumos"))
...
def test_lumos_ibc_transfer(ibc):
    """
...
def test_lumos_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending alumos from lumos to crypto-org-chain using cli
