...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network lumos', 'run all tests using lumos network')
    .example(
      '$0 --network lumos --allowTests=test1,test2',
      'run only test1 and test2 using lumos network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|lumos')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print lumosd output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'lumos' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or lumos')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-lumos']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-lumos'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'lumos') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the lumos process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'lumosd started'

    const lumosdProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting lumosd process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      lumosdProc.stdout.pipe(process.stdout)
    }

    lumosdProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start lumosd timeout!')), timeout)
  })
