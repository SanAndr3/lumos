...
def setup_lumos(path, base_port, long_timeout_commit=False):
    ...
    yield from setup_custom_lumos(path, base_port, cfg)
...
def create_snapshots_dir(
    ...
    for idx in range(n_nodes):
        data_snapshots_dir = path / "lumos_7798-1" / f"node{idx}" / "data" / "snapshots"
        os.makedirs(data_snapshots_dir, exist_ok=True)


def setup_lumos_rocksdb(path, base_port, long_timeout_commit=False):
    """
    setup_lumos_rocksdb returns an Lumos chain compiled with RocksDB
...
def setup_custom_lumos(
    ...
    try:
        ...
        yield Lumos(
            path / "lumos_7798-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
...
def build_patched_lumosd(patch_nix_file):
    """
    build the binary modified for a custom scenario
    e.g. allow to register WLUMOS token
