...
let
  ...
  ldflags = lib.concatStringsSep "\n" ([
    "-X github.com/cosmos/cosmos-sdk/version.Name=lumos"
...
buildGoApplication rec {
  ...
  subPackages = [ "cmd/lumosd" ];
  CGO_ENABLED = "1";

  postFixup = if dbBackend == "rocksdb" then
    ''
      # Rename the binary from lumosd to lumosd-rocksdb
      mv $out/bin/lumosd $out/bin/lumosd-rocksdb
    '' else '''';

  meta = with lib; {
    description = "Lumos is a scalable and interoperable blockchain, built on Proof-of-Stake with fast-finality using the Cosmos SDK which runs on top of CometBFT Core consensus engine.";
    homepage = "https://github.com/lumoschain/lumos";
