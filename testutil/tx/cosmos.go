...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 LUMOS
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'lumos_7798-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appLumos.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appLumos.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
