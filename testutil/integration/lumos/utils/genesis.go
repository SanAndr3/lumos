...
// the WLUMOS and the provided denoms.
...
func CreateGenesisWithTokenPairs(keyring testkeyring.Keyring, denoms ...string) network.CustomGenesisState {
	...
	for _, genesisAccount := range genesisAccounts {
		...
		// e.g. https://github.com/lumosnetwork/cosmos-sdk/blob/v0.47.5-lumos.2/x/auth/keeper/keeper_test.go#L193-L223
	...
	wlumosAddr := common.HexToAddress(erc20types.WLUMOSContractTestnet).Hex()

	// Add token pairs to genesis
	tokenPairs := make([]erc20types.TokenPair, 0, len(denoms)+1)
	// add token pair for fees token (wlumos)
	tokenPairs = append(tokenPairs, erc20types.TokenPair{
		Erc20Address:  wlumosAddr,
	...
	// with the WLUMOS (default is mainnet) and 'xmpl' tokens in the erc20 params
	erc20GenesisState.Params.NativePrecompiles = []string{wlumosAddr}
