	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"lumos_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "lumos_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"lumos_1": func(jt *JumpTable) {},
				"lumos_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "lumos_0", "lumos_1", "lumos_2"), eips, "expected eips number to be equal")
			},
		},
		{
