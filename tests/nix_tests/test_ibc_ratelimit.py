...
def ibc(request, tmp_path_factory):
    """
    prepare IBC network with an lumos chain
...
def test_lumos_ibc_transfer_native_denom(ibc, name, transfer_amt, err_contains):
    """
    test sending alumos from lumos to crypto-org-chain using cli.
    """
    assert_ready(ibc)
    ...
    if len(limits) == 0:
        add_rate_limit(lumos)

    old_src_balance = get_balance(lumos, src_addr, src_denom)
    old_dst_balance = get_balance(chainmain, dst_addr, LUMOS_IBC_DENOM)
...
def test_lumos_ibc_transfer_ibc_denom(
    ...
    old_dst_balance = get_balance(lumos, src_addr, src_denom)
    ...
    if err_inflow is not None:
        ...
        new_dst_balance = get_balance(lumos, src_addr, src_denom)
        assert new_dst_balance == old_dst_balance
        return

    def check_balance_change_lumos():
        new_dst_balance = get_balance(lumos, src_addr, src_denom)
        return old_dst_balance < new_dst_balance

    wait_for_fn("balance change", check_balance_change_lumos)

    # submit proposal if limit was not set
    limits = cli.rate_limits()
    # expect to already have one rate limit (for 'alumos') from the previous test
    if len(limits) == 1:
        add_rate_limit(lumos, src_denom)
...
def add_rate_limit(lumos: Lumos, denom: str = "alumos"):
    cli = lumos.cosmos_cli()
