...
@pytest.fixture(scope="module", params=["lumos"])
def ibc(request, tmp_path_factory):
    """
    Prepares the network.
    """
    name = "ibc-precompile"
    lumos_build = request.param
    path = tmp_path_factory.mktemp(name)
    network = prepare_network(path, name, [lumos_build, "chainmain"])
    yield from network
...
def test_denom_trace(ibc, name, args, err_contains, exp_res):
    ...
    # setup: send some funds from chain-main to lumos
    ...
    try:
        # make the query call
        pc = get_precompile_contract(ibc.chains["lumos"].w3, "ICS20I")
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["community"],
                [["transfer", "channel-0", [["alumos", int(1e18)]], [], []]],
            ],
            [],
            "improper number of arguments",
            None,
        ...
        (
            ...
            None,
            [("transfer", "channel-0", [("alumos", 1000000000000000000)], [], [])],
        ),
        (
            ...
            [
                ...
                [
                    [
                        "transfer",
                        "channel-0",
                        [["alumos", int(1e18)], ["uatom", int(1e18)]],
                        [],
                        [],
                    ]
                ],
            ...
            [
                (
                    "transfer",
                    "channel-0",
                    [("alumos", 1000000000000000000), ("uatom", 1000000000000000000)],
                    [],
                    [],
                )
            ],
...
def test_query_allowance(ibc, name, auth_args, args, err_contains, exp_res):
    ...
    gas_limit = 200_000
    pc = get_precompile_contract(ibc.chains["lumos"].w3, "ICS20I")
    lumos_gas_price = ibc.chains["lumos"].w3.eth.gas_price
    ...
    approve_tx = pc.functions.approve(*auth_args).build_transaction(
        {
            ...
            "gasPrice": lumos_gas_price,
            "gas": gas_limit,
        }
    )
    tx_receipt = send_transaction(ibc.chains["lumos"].w3, approve_tx, KEYS["validator"])
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-1", [["alumos", int(1e18)]], [], []]],
            ],
            True,
            "channel not found",
            0,
        ...
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["alumos", MAX_UINT256]], [], []]],
            ],
            False,
            "",
            MAX_UINT256,
...
def test_approve(ibc, name, args, exp_err, err_contains, exp_spend_limit):
    ...
    tx_receipt = send_transaction(ibc.chains["lumos"].w3, approve_tx, KEYS["signer1"])
    if exp_err:
        assert tx_receipt.status == 0, f"Failed: {name}"
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["lumos"], tx_receipt.transactionHash.hex())
    ...
    assert auth_event.args.allocations[0] == (
        "transfer",
        "channel-0",
        [("alumos", exp_spend_limit)],
        [],
        [],
    )

    # check the authorization was created
    cli = ibc.chains["lumos"].cosmos_cli()
...
def test_revoke(ibc, name, args, exp_err, err_contains):
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["lumos"].w3, revoke_tx, KEYS["signer1"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            "",
            json.loads("""[{"denom": "alumos", "amount": "2000000000000000000"}]"""),
        ),
        (
            ...
            "",
            json.loads(
                """[{"denom": "alumos", "amount": "1000000000000000000"},"""
...
def test_increase_allowance(
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["lumos"].w3, incr_allowance_tx, KEYS["validator"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "decrease allowance by 0.5 LUMOS",
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["alumos", int(1e18)]], [], []]],
            ],
            ...
            False,
            "",
            json.loads("""[{"denom": "alumos", "amount": "500000000000000000"}]"""),
        ),
        (
...
def test_decrease_allowance(
    ...
    tx_receipt = send_transaction(ibc.chains["lumos"].w3, approve_tx, KEYS["community"])
    assert tx_receipt.status == 1
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["lumos"].w3, decr_allowance_tx, KEYS["community"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "transfer 1 Lumos from chainA to chainB and spend the entire allowance",
            [["alumos", int(1e18)]],
            [
                "transfer",
                "channel-0",
        ...
        (
            "transfer 1 Lumos from chainA to chainB and don't change the unlimited spending limit",
            [["alumos", MAX_UINT256]],
            [
                "transfer",
                "channel-0",
            ...
            None,
            int(1e18),
            json.loads(f"""[{{"denom": "alumos", "amount": "{MAX_UINT256}"}}]"""),
        ),
        (
            "transfer 1 Lumos from chainA to chainB and only change 1 spend limit",
            [["alumos", int(1e18)], ["uatom", int(1e18)]],
            [
                "transfer",
                "channel-0",
...
def test_ibc_transfer_with_authorization(
    ...
    if auth_coins is not None:
        ...
        receipt = send_transaction(ibc.chains["lumos"].w3, approve_tx, KEYS["signer2"])
    ...
    src_amount_lumos_prev = get_balance(ibc.chains["lumos"], src_address, src_denom)
    dst_balance_prev = get_balance(
        ibc.chains["chainmain"], dst_address, LUMOS_IBC_DENOM
    )
    try:
        ...
        receipt = send_transaction(ibc.chains["lumos"].w3, transfer_tx, KEYS["signer2"])
    except Exception as err:
        check_error(err, err_contains)
        return
    ...
    if exp_err:
        ...
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["lumos"], receipt.transactionHash.hex())
    ...
    assert receipt.status == 1, debug_trace_tx(
        ibc.chains["lumos"], receipt.transactionHash.hex()
    )
    fees = receipt.gasUsed * lumos_gas_price
    ...
    assert transfer_event.args.denom == "alumos"
    ...
    src_amount_lumos_final = get_balance(ibc.chains["lumos"], src_address, src_denom)

    assert src_amount_lumos_final == src_amount_lumos_prev - fees - transfer_amt
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_adr = ibc.chains["lumos"].cosmos_cli().address("signer2")
    ...
    tx_receipt = send_transaction(ibc.chains["lumos"].w3, approve_tx, KEYS["signer2"])
    assert tx_receipt.status == 1
    ...
    src_starting_balance = get_balance(ibc.chains["lumos"], src_adr, "alumos")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, LUMOS_IBC_DENOM
    )
    # Calling the actual transfer function on the custom contract
    send_tx = eth_contract.functions.transferFromEOA(
        "transfer", "channel-0", src_denom, amt, dst_addr
    ).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": lumos_gas_price, "gas": gas_limit}
    )
    receipt = send_transaction(ibc.chains["lumos"].w3, send_tx, KEYS["signer2"])
    ...
    src_final_amount_lumos = get_balance(ibc.chains["lumos"], src_adr, src_denom)
    assert src_final_amount_lumos == src_starting_balance - amt - fees
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    src_denom = "alumos"
    gas_limit = 200_000
    lumos_gas_price = w3.eth.gas_price

    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    # address that will escrow the alumos tokens when transferring via IBC
    escrow_bech32 = lumos.cosmos_cli().escrow_address(channel)
    ...
    eth_contract = setup_interchain_sender_contract(
        lumos, ACCOUNTS["signer2"], amt, initial_contract_balance
    )

    # get starting balances to check after the tx
    src_bech32 = lumos.cosmos_cli().address("signer2")
    contract_bech32 = eth_to_bech32(eth_contract.address)
    other_addr_bech32 = eth_to_bech32(other_addr)

    src_starting_balance = get_balance(ibc.chains["lumos"], src_bech32, "alumos")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, LUMOS_IBC_DENOM
    )
    other_addr_initial_balance = get_balance(
        ibc.chains["lumos"], other_addr_bech32, src_denom
    )
    escrow_initial_balance = get_balance(ibc.chains["lumos"], escrow_bech32, src_denom)
    ...
    if other_addr == src_addr:
        ...
        other_addr_final_balance = get_balance(
            ibc.chains["lumos"], escrow_bech32, src_denom
        )
    ...
    src_final_amount_lumos = get_balance(ibc.chains["lumos"], src_bech32, src_denom)
    assert src_final_amount_lumos == exp_src_final_bal, f"Failed {name}"

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["lumos"], contract_bech32, src_denom
    )
    assert (
    ...
    escrow_final_balance = get_balance(ibc.chains["lumos"], escrow_bech32, src_denom)
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = lumos.cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    eth_contract = setup_interchain_sender_contract(
        lumos, ACCOUNTS["signer2"], ibc_transfer_amt, initial_contract_balance
    )

    # send some funds (100 alumos) to the contract to perform
    # internal transfer within the tx
    src_bech32 = eth_to_bech32(src_addr)
    contract_bech32 = eth_to_bech32(eth_contract.address)

    # get starting balances to check after the tx
    src_starting_balance = get_balance(lumos, src_bech32, "alumos")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, LUMOS_IBC_DENOM
    )
    escrow_initial_balance = get_balance(lumos, escrow_bech32, src_denom)
...
def test_multi_ibc_transfers_with_revert(ibc):
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = ibc.chains["lumos"].cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    interchain_sender_contract = setup_interchain_sender_contract(
        lumos, ACCOUNTS["signer2"], ibc_transfer_amt * 2, initial_contract_balance
    )
    ...
    assert src_final_amount_lumos == exp_src_final_bal

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["lumos"], interchain_sender_contract_bech32, src_denom
    )
    assert (
        contract_final_balance
        == initial_contract_balance - exp_amt_tranferred_internally
...
def setup_interchain_sender_contract(
    lumos, src_acc, transfer_amt, initial_contract_balance
):
    """
    ...
    src_denom = "alumos"
    gas_limit = 200_000
    lumos_gas_price = lumos.w3.eth.gas_price
    # Deployment of contracts and initial checks
    eth_contract, tx_receipt = deploy_contract(lumos.w3, CONTRACTS["InterchainSender"])
    assert tx_receipt.status == 1

    counter = eth_contract.functions.counter().call()
    assert counter == 0

    pc = get_precompile_contract(lumos.w3, "ICS20I")
    # Approve the contract to spend the src_denom
    approve_tx = pc.functions.approve(
    ...
    tx_receipt = send_transaction(lumos.w3, approve_tx, src_acc.key)
    assert tx_receipt.status == 1
    ...
    contract_bech32 = eth_to_bech32(eth_contract.address)
    fund_tx = lumos.cosmos_cli().transfer(
        src_bech32,
        contract_bech32,
        f"{initial_contract_balance}alumos",
        gas_prices=f"{lumos_gas_price + 100000}alumos",
        generate_only=True,
    )

    fund_tx = lumos.cosmos_cli().sign_tx_json(fund_tx, src_bech32, max_priority_price=0)
    rsp = lumos.cosmos_cli().broadcast_tx_json(fund_tx, broadcast_mode="sync")
    assert rsp["code"] == 0, rsp["raw_log"]
    txhash = rsp["txhash"]
    wait_for_new_blocks(lumos.cosmos_cli(), 2)
    receipt = lumos.cosmos_cli().tx_search_rpc(f"tx.hash='{txhash}'")[0]
