...
import (
	...
	"github.com/lumoschain/lumos/v20/types"
	...
	commonnetwork "github.com/lumoschain/lumos/v20/testutil/integration/common/network"
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new LumosApp with the following params
	lumosApp := createLumosApp(n.cfg.chainID, n.cfg.customBaseAppOpts...)
	...
	slashingParams, err := getValidatorsSlashingGen(validators, lumosApp.StakingKeeper)
	if err != nil {
		return err
	}
	...
	genesisState, err = customizeGenesis(lumosApp, n.cfg.customGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	header := cmtproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             lumosApp.LastBlockHeight() + 1,
		AppHash:            lumosApp.LastCommitID().Hash,
		Time:               now,
	...
	if _, err := lumosApp.FinalizeBlock(req); err != nil {
		return err
	}

	// TODO - this might not be the best way to initilize the context
	n.ctx = lumosApp.BaseApp.NewContextLegacy(false, header)

	// Commit genesis changes
	if _, err := lumosApp.Commit(); err != nil {
		return err
	}
