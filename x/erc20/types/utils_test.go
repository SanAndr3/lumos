	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "Lumos",
				Symbol:      "LUMOS",
				Description: "EVM, staking and governance denom of Lumos",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "alumos",
						Exponent: 0,
						Aliases:  []string{"atto lumos"},
					},
					{
						Denom:    "lumos",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			[]string{"atto lumos", "alumos"},
			[]string{"alumos", "atto lumos"},
			false,
		},
		{
			"same values, sorted",
