...
from .network import setup_custom_lumos, setup_lumos
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_lumos(tmp_path_factory):
    ...
    yield from setup_lumos(path, 26200)


@pytest.fixture(scope="module")
def lumos_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module",
    params=["lumos", "geth", "lumos-ws", "lumos-rocksdb", "enable-indexer"],
)
def cluster(request, custom_lumos, lumos_rocksdb, lumos_indexer, geth):
    """
    run on both lumos and geth
    """
