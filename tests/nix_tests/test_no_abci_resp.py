...
from .network import create_snapshots_dir, setup_custom_lumos
...
def test_gas_eth_tx(lumos_cluster):
    """
    When node does not persist ABCI responses
    eth_gasPrice should return an error instead of crashing
    """
    wait_for_block(lumos_cluster.cosmos_cli(), 3)
    try:
        lumos_cluster.w3.eth.gas_price  # pylint: disable=pointless-statement
