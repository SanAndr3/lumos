...
import (
	...
	"github.com/lumoschain/lumos/v20/crypto/hd"

	"github.com/lumoschain/lumos/v20/server/config"
...
type Config struct {
	...
	GenesisState      lumostypes.GenesisState // custom gensis state to provide
...
func DefaultConfig() Config {
	chainID := fmt.Sprintf("lumos_%d-1", cmtrand.Int63n(9999999999999)+1)
	dir, err := os.MkdirTemp("", "simapp")
	if err != nil {
		panic(fmt.Sprintf("failed creating temporary directory: %v", err))
	}
	defer os.RemoveAll(dir)
	app := app.NewLumos(log.NewNopLogger(), dbm.NewMemDB(), nil, true, nil, dir, 0, simutils.NewAppOptionsWithFlagHome(dir), baseapp.SetChainID(chainID))
	return Config{
		Codec:             app.AppCodec(),
		...
		NumValidators:     4,
		BondDenom:         "alumos",
		MinGasPrices:      fmt.Sprintf("0.000006%s", lumostypes.AttoLumos),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, lumostypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, lumostypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, lumostypes.PowerReduction),
...
// NewAppConstructor returns a new Lumos AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !lumostypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "lumosd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "lumoscli")
		...
		genAccounts = append(genAccounts, &lumostypes.EthAccount{
		...
		customAppTemplate, _ := config.AppConfig(lumostypes.AttoLumos)
