...
const DefaultFeeAmt = int64(150_000_000_000_000_000) // 0.15 LUMOS

var globalStartTime = time.Date(2020, 1, 2, 0, 0, 0, 0, time.UTC)

// NewCoordinator initializes Coordinator with N EVM TestChain's (Lumos apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if lumosChain, ok := chain.App.(*app.Lumos); ok {
		bondDenom, err = lumosChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
