...
HTTPS_GIT := https://github.com/lumoschain/lumos.git
...
# Deps
DEPS_COSMOS_SDK_VERSION := $(shell cat go.sum | grep -E 'github.com/lumosnetwork/cosmos-sdk\s' | grep -v -e 'go.mod' | tail -n 1 | awk '{ print $$2; }')
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=lumos \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(LUMOS_BINARY) \
...
build-docker-goleveldb:
	...
	mkdir -p ./build/.lumosd
	echo '#!/usr/bin/env bash' > ./build/lumosd
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/lumosd
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/lumosd
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.lumosd:/home/lumos/.lumosd $$IMAGE_NAME lumosd "$$@"' >> ./build/lumosd
	chmod +x ./build/lumosd
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/lumoschain/lumos"
	godoc -http=:6060
...
proto-download-deps:
	...
	git init && \
	git remote add origin "https://github.com/lumosnetwork/cosmos-sdk.git" && \
...
PACKAGE_NAME:=github.com/lumoschain/lumos
...
# Lumos repository and then clean up the contracts data.
...
# Compile the solidity contracts found in the Lumos repository.
contracts-compile:
